...
* チャンク化により転送データ量が **1/8** に削減されるため、Chunked-Static の CPU 前処理が約 2.1ms 高速化する。

#### ② CPU前処理時間の差: Chunked-Dynamic (+3.3ms) vs Chunked-Static
* **原因**: Dynamic 固有の **$O(n_{lines})$ 文字数均等化スキャン** が追加されるためである。
* 全 831,111 行の文字数を CPU で順次累積ループ処理するため、前処理コストが大きく増加する。

#### ③ GPU実行時間の平均値の差: GPU Line (40.4ms) < Chunked-Static (42.3ms)
* 一見、スレッド数が少ない Chunked-Static の方が速そうに見えるが、**「直列処理の損」** が影響している。
  * **GPU Line (8スレッド並列)**: 8行の完了時間 $\approx \max(\text{8行の長さ})$
  * **Chunked-Static (1スレッド直列)**: 8行の完了時間 $\approx \sum(\text{8行の長さ})$
* 全 30 パターンの平均では、スレッド数が 1/8 に減るメリット（wave 数減）と、1スレッドの処理時間が伸びるデメリットが相殺し、平均では GPU Line がわずかに勝つ結果となった。
...
* **メカニズム**:  
  多分岐などの計算密度が高いパターンでは、1文字進むごとの NFA 状態遷移（`clist` $\to$ `nlist`）の計算が重くなる。  
  計算が重くなると、GPU Line の「スレッド/ワープ起動やメタデータロードなどの固定管理コスト」に対する「実質計算時間」の割合が増大する。  
  Chunked-Static (LPC=8) は管理コストを 1/8 に希釈しているため、**計算が重い複雑なパターンほど管理コスト削減効果が爆発的に効いて GPU Line を圧倒** する。
...
* **メカニズム**:  
  `.+` 等を含むパターンは一度マッチ状態に入ると行末まで走査を継続するため、行長に依存した遅延（Long Tail）を発生させる。  
  Chunked-Static (1スレッド8行直列) で長行に当たると、そのスレッドの完了を同じワープ内の他スレッドが待たされる **「ワープダイバージェンス (Warp Divergence)」** が深刻化する。  
  GPU Line は 1行1スレッドの完全並列度で並列計算するため、長行による遅延を並列度で押し潰すことができる。
