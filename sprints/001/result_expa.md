...
全体平均ではほぼ同等だが、**全 30 パターンを個別に切り分けると、最大 1.4〜1.6 倍の明確な差がつき、勝敗が 15 勝 15 敗に二分** されていた。
...
* **考察（NFA状態遷移の複雑性とL1キャッシュ・レジスタ再利用効率）**:  
  選択肢が多い複雑なパターンでは、1文字走査あたりの NFA 状態遷移関数 $\delta(Q, \Sigma) \to \mathcal{P}(Q)$ の評価に伴うアクティブ状態集合（`clist`, `nlist`, `visited` 配列）のサイズが膨大になり、スレッド内の作業領域が巨大化する。  
  GPU Line（1行1スレッド = 83万回起動）では、1行（約115文字）の処理ごとにスレッドが終了するため、GPU コア（SM）上の L1/L2 キャッシュやレジスタにウォームアップされた巨大な作業領域が破棄され、毎行一からメモリロードレイテンシを被る。  
  一方、Chunked-Static（1スレッド8行直列）では、1つのスレッドが SM 上のレジスタや L1 キャッシュに載った状態データ・作業配列を保持したまま 8 行分を連続ループ処理できる。正規表現が複雑で作業領域が大きいほど、この**時間的局所性（Temporal Locality）に基づくキャッシュ・レジスタの再利用効果**が絶大となり、実効メモリスループット向上によって GPU Line を大幅に上回ったと考えられる。
...
* **考察（ワープダイバージェンスとテールレイテンシ）**:  
  `.+` 等を含むパターンは、一旦マッチ状態に入ると行末まで状態を維持して文字走査を継続するため、処理時間が行長 $L_i$ に強く依存するテールレイテンシ（Tail Latency）を発生させる。  
  GPU の SIMT 実行モデルにおいて、同一ワープ（32スレッド）内の各スレッドは最長実行スレッドの完了を待機させられる（Warp Divergence）。  
  Chunked-Static の場合、1スレッドが 8 行分を直列に合算したチャンク長 $T_{\text{chunk}} = \sum_{k=1}^8 L_k$ を処理するため、8行合算によってチャンク長の MIN と MAX の格差が著しく拡大し、ワープ内の最長スレッドと最短スレッドの実行時間差（テールレイテンシ）が増幅される。  
  これに対し GPU Line は各スレッドの処理範囲が単一行長 $L_i$ に留まり、高い並列度（1行1スレッド）によってテールレイテンシによる待機オーバーヘッドを効果的に隠蔽（Latency Hiding）できていると考えられる。
...
## 4. 結論とまとめ
1. 各手法についてCPU/GPU実行時間を切り分けて確認すると、意外なことにstaticはCPU実行時間でLineに対して有利/GPU実行時間では劣後という結果だった。  
  1a. **最適パラメータ**: LPCを小さくするとむしろGPU実行時間が伸びることが確認された。GPU実行時間の劣後はLPCが大きすぎることによるとする仮説は否定された。  
2. 正規表現によってStatic/Lineの優劣が異なることがわかった。ORなどが多く状態遷移数が多い場合においてはStaticが有利であり、単純な正規表現においてはLineが有利だった。  
