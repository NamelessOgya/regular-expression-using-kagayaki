...
> **考察 (NFA状態遷移の複雑性と計算密度)**: 選択肢が多い複雑なパターンでは、1文字走査あたりの NFA 状態遷移関数 $\delta(Q, \Sigma) \to \mathcal{P}(Q)$ の評価コストが高まりスレッド内の計算密度が増大する。GPU Line のスレッド管理固定コストに対するアモタイゼーション（償却）効果により、LPC=8 で管理コストを 1/8 に削減する Chunked-Static が命令スループットで圧倒したと考えられる。
