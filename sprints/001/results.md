...
> **考察 (ワープダイバージェンスとテールレイテンシ)**: `.+` 等のワイルドカードは行長依存のテールレイテンシ（Tail Latency）を発生させる。Chunked-Static (8行直列) ではチャンク長 $T_{\text{chunk}} = \sum_{k=1}^8 L_k$ の分散加算により SIMT 実行におけるワープダイバージェンスが深刻化するのに対し、GPU Line は単一行長 $L_i$ に留まる高い並列度で遅延を隠蔽（Latency Hiding）できていると考えられる。
