...
### 考察: GPU 実行時間が Line < Static < Dynamic になる理由

**enwik8 の GPU 実行時間（再掲）:**

| 手法 | GPU 実行(ms) | wave 数 |
|---|---|---|
| GPU Line | **40.4ms** | 2.39 波 |
| Chunked-Static | 42.3ms | 0.30 波 |
| Chunked-Dynamic | 46.5ms | 0.30 波 |

Chunked-Static は wave 数が 1/8 なのに GPU 実行が遅いのは一見矛盾する。

#### Line vs Chunked-Static: 「wave 数の得」と「直列処理の損」の相殺

Chunked-Static（LPC=8）は Line と比べて：

- **得**: wave 数が 2.39 → 0.30（1/8）に減る
- **損**: 各スレッドが 8 行を**直列**に処理する → 1 スレッドの処理時間が「並列 max」から「直列 sum」へ

```
同じ 8 行を処理する場合の時間（行長が指数分布に従うと仮定）:

GPU Line（8スレッド並列）:
  かかる時間 ≈ max(8行) ≈ avg × 2.7（統計的期待値）

Chunked-Static（1スレッド直列）:
  かかる時間 ≈ sum(8行) = avg × 8.0

「直列の損」= 8.0 / 2.7 ≈ 3 倍のスレッド実行時間増
「wave 数の得」= 1 / 8 倍のラウンド数減

得（1/8）× 損（×3）→ トータルで Static が約 1.05 倍遅い（実測値と整合）
```

#### LPC を変えると優劣が逆転する

| LPC | wave 数の得 | 直列処理の損 | Static が有利? |
|---|---|---|---|
| 1 | × 1（同じ） | × 1（同じ） | 同等（= GPU Line） |
| 8 | × 1/8 | × ~3 | わずかに Line が有利 |
| 32 | × 1/32 | × ~5 | **Static が有利** |
...
### 考察: 正規表現パターンごとの詳細分岐 (NFA計算密度 vs ワープダイバージェンス)
...
> **考察 (NFA状態遷移の複雑性とL1キャッシュ・レジスタ再利用効率)**: 選択肢が多い複雑なパターンでは、NFA 状態遷移関数 $\delta(Q, \Sigma) \to \mathcal{P}(Q)$ の評価に伴う作業領域（アクティブ状態集合配列）が巨大化する。GPU Line（83万スレッド）では1行（約115文字）ごとにスレッドが終了して L1/L2 キャッシュ上の作業領域が破棄されるのに対し、Chunked-Static（10万スレッド）では 1 つのスレッドが SM 上のキャッシュ・レジスタに載った作業領域を保持したまま 8 行連続ループ処理できる。この時間的局所性（Temporal Locality）に基づく再利用効果が絶大となり、結果として命令・メモリスループットで圧倒したと考えられる。
...
> **考察 (ワープダイバージェンスとテールレイテンシ)**: `.+` 等のワイルドカードは行長依存のテールレイテンシ（Tail Latency）を発生させる。Chunked-Static (8行直列) ではチャンク長 $T_{\text{chunk}} = \sum_{k=1}^8 L_k$ の合算により MIN と MAX のチャンク長格差が拡大し、SIMT 実行におけるワープダイバージェンスが深刻化するのに対し、GPU Line は単一行長 $L_i$ に留まる高い並列度で遅延を隠蔽（Latency Hiding）できていると考えられる。


## 3'. 実験 A': LPC スイープによる仮説検証（enwik8, 3回平均）

**目的**: 「LPC を小さくするほど Chunked-Static/Dynamic の GPU 実行時間が短くなる」という仮説を検証する。  
**データ**: `wiki_plain.txt`（95,909,488 文字）  
**実行環境**: Docker コンテナ内（`re_exp_env_gpu`, `--gpus all`）  
**実行スクリプト**: `sprints/001/run_exp_a_prime.sh`  
**結果ディレクトリ**: `results/sprint001_exp_a_prime/`

### 計測結果（LPC=1〜32, CPU前処理 / GPU実行 / 合計 内訳）
...
**図: LPC スイープ — GPU 実行時間 & 合計時間**

![fig_exp_a_prime](figures/fig_exp_a_prime.png)

### 考察: 仮説は棄却された — LPC を大きくするほど GPU 実行が速い

**仮説**: LPC↓ → 直列処理↓ → GPU 実行↓  
**実測**: LPC=1 が最遅（Static: 44.4ms）、LPC=8 が最速（41.6ms）→ **仮説と逆の傾向**

#### 原因: Chunk カーネルの固定オーバーヘッド

GPU Line カーネルと Chunked-Static カーネルは**別実装**であり、チャンクカーネルはチャンク境界の参照・管理などの**固定オーバーヘッド**を持つ。

```
LPC=1（831K チャンク × 2.4 波）:
  各スレッドの仕事量 = 1行 + チャンクオーバーヘッド
  → オーバーヘッドが全行数分かかる

LPC=8（104K チャンク × 0.3 波）:
  各スレッドの仕事量 = 8行 + チャンクオーバーヘッド
  → オーバーヘッドが 1/8 の頻度で済む（= 8行に1回償却）
```

LPC=1 の Static は GPU Line と同じ行数を処理するが、チャンクカーネルの余分なオーバーヘッドがあるため **GPU Line より約 4ms 遅い**（44.4ms vs 40.4ms）。

#### LPC を大きくするほど良い理由のまとめ

| LPC | wave 数 | 1スレッドの直列仕事 | カーネルオーバーヘッド密度 | GPU実行 |
|---|---|---|---|---|
| 1 | 2.4 波 | 1行（少） | 高（行ごと） | 44.4ms（遅） |
| 4 | 0.6 波 | 4行（中） | 中（4行に1回） | 44.8ms |
| 8 | 0.3 波 | 8行（多） | 低（8行に1回）| **41.6ms（速）** |

> [!NOTE]
> 「直列処理の損」よりも「カーネルオーバーヘッドの償却」の効果の方が大きい（LPC=1〜8 の範囲では）。  
> より大きな LPC（16, 32）では直列処理コストが増大して再び遅くなる可能性があり、最適点が存在するはず。  
> その探索が実験 C（LPC スイープ, varied データ）の目的であった。
